...
excel_path = os.path.join(testdata_dir, "similarity_cases.xlsx")   # 获取文件路径
me = MyExcel(excel_path, "类案同判库")  # 读取文件
case_list = me.read_all_datas()  # 将读取到的内容通过key，value的格式组成字典，存入列表

# 3-执行用例
@ddt
class similarity(BaseCase):
    @list_data(case_list)
    def test_similarity(self, case):
        self.base_case(case)