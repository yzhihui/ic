...
excel_path = os.path.join(testdata_dir, "login_cases.xlsx")   # 获取文件路径
me = MyExcel(excel_path, "登录接口测试用例")  # 读取文件
case_list = me.read_all_datas()  # 将读取到的内容通过key，value的格式组成字典，存入列表


# 3-执行用例
@ddt
class AdminLogin(BaseCase):
    @list_data(case_list)
    def test_admin_login(self, case):
        self.base_case(case)